...
- Replaced boxed enum controls and transient inspector labels with reusable editor data.
- Declared Unity 2022.3 and Shader Graph 14.0.7 as the minimum supported versions.
- Preserved the registered demo at its existing `Samples` path to avoid duplicate assets during package updates.

### Fixed

- Disabled depth writes for the generated Built-in and URP skybox passes.
